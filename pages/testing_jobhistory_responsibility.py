#job history score calculated based on each responsibility vs overall job description
...
def calculate_similarity_scores(resume_texts, filenames, job_desc_embedding, skill_embeddings, adjusted_skill_weights, embeddings_model, qa_chain):
    ...
    for idx, resume_text in enumerate(resume_texts):
        ...
        - Work experiences (list of job experiences), where each job experience includes:
            ...
            - Start date
            - End date
            - Responsibilities (extract 3 most important work done and skill used for the particular role )
        ...
        {{
            ...
            "Work experiences": [
                {{"Job title": "Data Scientist", "Company name": "ABC Corp", "Start date": "Jan 2020", "End date": "Present", "Responsibilities": ["Developed AI models for predictive analytics.", "Collaborated with cross-functional teams to implement data-driven solutions.","Improved model accuracy by using ensemble model"]}},
                {{"Job title": "Junior Developer", "Company name": "XYZ Ltd", "Start date": "May 2017", "End date": "Dec 2019", "Responsibilities": ["Assisted in developing web applications.", "Maintained databases and ensured data integrity.","Customised IT solutions"]}}
            ]
        }}
        Return only the JSON data. Do not include any additional text.
...
if st.button("Visualise Relevancy of Past Experience"):
    if 'results' in st.session_state:
        ...
        for idx, row in df_results.iterrows():
            ...
            candidate_id = row.get('Filename', f"Candidate_{idx+1}")  # Use Filename as Candidate ID
        ...
        if all_experiences:
            ...
            # Interactive Filter: Select Candidates (Optional)
            candidate_options = df_experiences['Candidate ID'].unique().tolist()
            selected_candidates = st.multiselect("Select Candidates to Display:", options=candidate_options, default=candidate_options)
            
            if selected_candidates:
                df_filtered = df_experiences[df_experiences['Candidate ID'].isin(selected_candidates)]
            else:
                df_filtered = df_experiences  # If none selected, show all
            ...
            fig_line = px.line(
                ...
                # Remove Job Title and Company since they're no longer relevant
                hover_data=[]  # Alternatively, you can include other relevant data if available
            )
